...
void sendTelemetry() { // AFTER UPDATE
    ...
    telemetryJsonPacket += "{\"level\":"; // AFTER UPDATE
...
// --- Task 8: WiFi WebSocket Telemetry --- // AFTER UPDATE
    ...
    for (;;) { // AFTER UPDATE
        ...
        if (telemetryWebSocketStarted && WiFi.status() == WL_CONNECTED) { // AFTER UPDATE
            ...
            if (now - telemetryLastDebugPrint >= 3000) { // AFTER UPDATE
                ...
                Serial.printf("[TEL] clients=%d packets=%lu interval=%lums ip=%s\n", webSocket.connectedClients(), telemetryPacketsSent, TELEMETRY_INTERVAL_MS, WiFi.localIP().toString().c_str()); // AFTER UPDATE
            } // AFTER UPDATE
        } // AFTER UPDATE
