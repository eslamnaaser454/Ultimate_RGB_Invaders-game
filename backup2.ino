...
void onWebSocketEvent(uint8_t clientId, WStype_t type, uint8_t *payload, size_t length) { // AFTER UPDATE
    if (type == WStype_CONNECTED) { // AFTER UPDATE
        ...
        if (strcmp(pktType, "command") == 0) { // AFTER UPDATE
            ...
            if (strcmp(action, "LEFT") == 0 || strcmp(action, "FIRE") == 0) g_virtualInput.blue = true; // AFTER UPDATE
            if (strcmp(action, "RIGHT") == 0 || strcmp(action, "COMBO") == 0) g_virtualInput.red = true; // AFTER UPDATE
            if (strcmp(action, "FIRE") == 0) g_virtualInput.green = true; // AFTER UPDATE
